...
pub trait ClassDef {
    ...
    fn init_proto<'js>(ctx: Ctx<'js>, proto: &Object<'js>) -> Result<()> {
        ...
        proto.prop(
            to_string_tag,
            Property::from(Self::CLASS_NAME).configurable(),
        )?;
        Ok(())
    }
...
impl<'js, 't, E> Outlive<'t> for ErrorClass<'js, E> {
    type Target = ErrorClass<'t, E>;
}

impl<'js, E> Deref for ErrorClass<'js, E> {
...
impl<'js, E> AsRef<Object<'js>> for ErrorClass<'js, E> {
    fn as_ref(&self) -> &Object<'js> {
        &self.0
    }
}

impl<'js, E> AsRef<Value<'js>> for ErrorClass<'js, E> {
...
pub struct ErrorConstructor<E, F>(pub(crate) F, PhantomData<E>);

impl<E, F> AsRef<F> for ErrorConstructor<E, F> {
    fn as_ref(&self) -> &F {
        &self.0
    }
}

impl<E, F> Deref for ErrorConstructor<E, F> {
    type Target = F;
